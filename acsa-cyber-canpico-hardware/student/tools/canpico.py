...
class CANPico:
    ...
    # Wait for a specific arbitration ID
    def wait_for_id(self, can_id, timeout_ms=5000):
        """Wait for a frame with a specific arbitration ID."""
        start = utime.ticks_ms()
        while utime.ticks_ms() - start < timeout_ms:
            frames = self.c.recv()
            for frame in frames:
                if frame.get_arbitration_id() == can_id:
                    return frame
        return None
    def recv_wait(self):
