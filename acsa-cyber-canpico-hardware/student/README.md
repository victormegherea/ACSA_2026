...
- **Termination:** 120 Ω between CAN-H and CAN-L at each physical end of the
  bus (use the board's built-in termination instead of an external resistor when
  it is enabled)
...
The vendor firmware reference is linked from `instructor/FIRMWARE_SETUP.md`; it
is intentionally kept out of the student package.
