...
| `ch.set_frame(can_id, data)` | Define the frame ID/payload to target or send | Ch.3-8 |
| `ch.spoof_frame(timeout, overwrite=False)` | Target an ID and inject the configured frame | Ch.3, 6, 8 |
| `ch.error_attack(repeat, timeout)` | Inject CAN error frames for the configured ID | Ch.4, 5, 6 |
| `ch.send_janus_frame(sync_time, split_time, retries)` | Send Janus frame | Ch.7 |
| `c.get_status()` | Read `(bus_off, error_passive, TEC, REC)` | Ch.4, 5, 8, 10 |
| `f.get_timestamp()` | Frame timestamp | Ch.9 |

`set_attack()`, `inject_error`, and `spoof_frame_error_passive()` are not part of
the CANPico SDK. `set_frame()` supplies the target arbitration ID; for spoofing it
also supplies the replacement payload. The examples use the official CANHack API.
