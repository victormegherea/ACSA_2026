...
The template has three detection heuristics with `TODO` markers. You must implement them before the IDS can detect the attacks:
...
### Step 3.2 — Implement and test the IDS against the attack log
```bash
python3 scripts/simple_ids.py --file logs/attack_trace.log
```

Before this command can detect attacks, implement the three TODO sections in
`scripts/simple_ids.py`. Then rerun the command. **Expected output:** the IDS
should flag multiple alerts for each attack type.
