...
This track uses the self-contained `scripts/seed_key.py` simulator. The
`scripts/uds_client.py` file is an optional single-frame client template and
requires a separate UDS responder; it is not needed for the required tasks.
...
UDS> key 0000
```
**Expected:** Invalid key (NRC 0x35). You have 3 attempts; the third failure returns NRC 0x36 and starts a 10-second lockout.
...
If you trigger the three-attempt lockout while experimenting, wait 10 seconds or use `reset` to restart the simulator.
...
- What happens after 3 failed attempts (NRC 0x36 and a 10-second lockout)
...
The ECU limits you to 3 attempts and then locks out for 10 seconds — so you need the algorithm.
</details>
