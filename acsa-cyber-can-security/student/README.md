...
│   └── uds_client.py     ← Optional single-frame UDS client template
├── logs/
...
Track C is taught with the self-contained `scripts/seed_key.py` virtual ECU.
`uds_client.py` is an optional extension for a separately running UDS responder;
it does not connect to the virtual ECU by itself.
