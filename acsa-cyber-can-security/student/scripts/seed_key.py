...
class VirtualECU:
    ...
    def security_access_request_seed(self):
        ...
        if self.lockout_until is not None:
            remaining = self.lockout_until - time.monotonic()
            if remaining > 0:
                return False, f"NRC 0x36: Exceeded number of attempts (wait {remaining:.0f}s)"
            self.lockout_until = None
            self.attempts = 0
    ...
    def security_access_send_key(self, key):
        ...
        if self.lockout_until is not None:
            remaining = max(0, self.lockout_until - time.monotonic())
            return False, f"NRC 0x36: Exceeded number of attempts (wait {remaining:.0f}s)"
        ...
        if key == expected_key:
            ...
            print(f"  [ECU] Wrong key 0x{key:04X}. Attempts remaining: {max(0, remaining)}")
            if self.attempts >= self.max_attempts:
                self.lockout_until = time.monotonic() + 10
                return False, "NRC 0x36: Exceeded number of attempts (wait 10s)"
    ...
    def read_data(self, did):
        ...
        if did in self.data_store:
            name, value = self.data_store[did]
    ...
    def write_data(self, did, value):
        ...
        name, _ = self.data_store[did]
        self.data_store[did] = (name, value)
...
def interactive_session():
    ...
    print("    reset              — Reset the ECU simulator")
    ...
    while True:
        ...
        if action == "quit" or action == "exit":
            ...
            if ecu.lockout_until is not None:
                print(f"  Lockout: {max(0, ecu.lockout_until - time.monotonic()):.0f}s remaining")
            ...
            ecu = VirtualECU()
            print("  [ECU] Simulator reset; session and security state cleared.")
