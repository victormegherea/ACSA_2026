...
Requires: python-can. This client implements only UDS single-frame requests;
an ISO-TP/UDS responder must be running separately on the configured CAN IDs.
...
class SimpleUDSClient:
    ...
    def send_request(self, data, timeout=2.0):
        ...
        if self.bus is None:
            raise RuntimeError("connect() must be called before send_request()")
        data = bytes(data)
        # ISO-TP single frames carry at most seven service bytes in an
        # eight-byte classic CAN payload.  Multi-frame requests need an
        # ISO-TP stack and are deliberately outside this teaching client.
        if not 1 <= len(data) <= 7:
            raise ValueError("single-frame UDS request must contain 1..7 bytes")
        if timeout <= 0:
    ...
    def _parse_response(self, data):
        ...
        if length > len(data) - 1:
            return {"status": "malformed", "error": "declared length exceeds payload"}
        ...
        if sid == 0x7F:
            if length < 3:
                return {"status": "malformed", "error": "short negative response"}
            rejected_sid = data[2]
            nrc = data[3]
        ...
        if sid < 0x40:
            return {"status": "malformed", "error": f"invalid response SID 0x{sid:02X}"}
        return {
            "status": "positive",
            "sid": sid - 0x40,
